...
from aws_solutions_constructs.aws_lambda_dynamodb import LambdaToDynamoDB                           # Toegevoegd voor L3 construct
...
# Start L3 construct
        products_backend = LambdaToDynamoDB(self, 'ProductsBackend',                                # Gehele products_backend variable toegevoegd voor L3 construct
                                            ...
                                            table_permissions='Read')                               # Hier wordt permission gegeven aan de Lambda om de DynamoDB tabel te lezen. Dit gebeurt in de L2 constructs mbv products_table.grant_read_data(product_list_function.role)
        
        products_table = products_backend.dynamo_table                                              # L3 versie van products_table

        product_list_function = products_backend.lambda_function                                    # L3 versie van product_list_function

        products_table.apply_removal_policy(RemovalPolicy.DESTROY)                                  # Default in deze L3 construct is om de DynamoDB tabel te laten bestaan wanneer de stack vernietigd wordt. Ik wil echter dat de tabel wel vernietigd wordt wanneer de stack vernietigd wordt dus stel ik hier een andere RemovalPolicy in. 
# Einde L3 construct
